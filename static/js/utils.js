...
function performGenericSearch(config) {
	...
	if (!searchTerm) {
		...
		if (
			...
			typeof window.filteredCollections !== "undefined"
		) {
	...
	try {
		...
		if (
			...
			window.filteredCollections = results
				.map((result) => collections[parseInt(result.ref, 10)])
		...
		results.slice(0, 10).forEach((result) => {
			const collection = collections[parseInt(result.ref, 10)];
			...
			const title = highlightSearchTerm(
				collection.title || "Untitled",
				searchTerm,
			);
			const sigla = collection.sigla
				? `<span class="search-result-sigla">${collection.sigla}</span>`
				: "";
		...
		searchResultsContainer.innerHTML =
			'<div class="p-3 text-center">Search error occurred</div>';
	}
}
...
function calculateBasicSearchScore(item, searchTerm) {
	...
	if (item.sigla?.toLowerCase().includes(searchLower)) {
		score += 10;
	}
	...
	if (
		item.alternative_title?.toLowerCase().includes(searchLower) ||
		item.title_tibetan?.toLowerCase().includes(searchLower)
	) {
		score += 3;
	}

	// Check place of production
	if (item.place_of_production?.toLowerCase().includes(searchLower)) {
		score += 2;
	}

	// Check genre/category
	if (item.genre?.toLowerCase().includes(searchLower)) {
		score += 2;
	}
...
function showError(message, containerId = "collections-container") {
	...
	if (container) {
		...
		console.error(
			`Error container with ID '${containerId}' not found. Error message: ${message}`,
		);
	}
}
...
function downloadCollectionXML(fileName) {
	...
	fetch(xmlFilePath)
		...
		.catch((error) => {
			...
			alert(
				`Error downloading XML file: ${error.message}\nPlease check the browser console for more details.`,
			);
		});
}
...
function updateGenericCategoryChart(
	...
	chartId,
	chartInstance,
	chartTitle = "Collections by Category",
) {
...
// Parses a date string into [startYear, endYear]. Returns null if unparseable.
// Handles: "17th cent.", "14th-17th cent.", "1684-1692", "ca. 1410".
function parseDateRange(dateStr) {
	...
	let match = dateStr.match(
		/(\d{1,2})(?:st|nd|rd|th)\s*-\s*(\d{1,2})(?:st|nd|rd|th)\s+cent/,
	);
	if (match) {
	...
	if (match) {
		...
		if (match[2].length === 2) {
			endYear = parseInt(match[1].substring(0, 2) + match[2], 10);
		}
		return [startYear, endYear];
	}
...
function filterCollections(collections, filters = {}, customFilterFn = null) {
	...
	if (filters.medium && filters.medium !== "all") {
		const mediaArr = Array.isArray(filters.medium)
			? filters.medium
			: [filters.medium];
		filtered = filtered.filter(
			(collection) => collection.medium && mediaArr.includes(collection.medium),
		);
	}

	// Apply date range filter if provided
	if (
		(filters.minYear !== undefined && filters.minYear !== null) ||
		(filters.maxYear !== undefined && filters.maxYear !== null)
	) {
		const minYear =
			filters.minYear !== undefined ? parseInt(filters.minYear, 10) : -Infinity;
		const maxYear =
			filters.maxYear !== undefined ? parseInt(filters.maxYear, 10) : Infinity;
