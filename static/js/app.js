...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (markerViewBtn && heatmapViewBtn) {
		markerViewBtn.addEventListener("click", () => {
			if (isHeatmapActive) {
				switchToMarkerView();
	...
	function toggleUndatedBasedOnRange() {
		const minVal = parseInt(dateSliderMin.value, 10);
		const maxVal = parseInt(dateSliderMax.value, 10);
	...
	if (dateSliderMin) {
		dateSliderMin.addEventListener("input", function () {
			...
			if (value > parseInt(dateSliderMax.value, 10)) {
		...
		dateSliderMin.addEventListener("change", () => {
	...
	if (dateSliderMax) {
		dateSliderMax.addEventListener("input", function () {
			const value = parseInt(this.value, 10);
			if (value < parseInt(dateSliderMin.value, 10)) {
		...
		// Apply filters immediately when slider stops
		dateSliderMax.addEventListener("change", () => {
			if (undatedAuto && includeUndatedCheckbox) {
				toggleUndatedBasedOnRange();
			}
			applyFilters();
...
function initMap() {
	...
	mapContainer.addEventListener("click", (_e) => {
...
async function loadData() {
	try {
		...
		const geoResponse = await fetch(
			`/static/data/collections.geojson${cacheBuster}`,
		);
...
function createSearchIndex() {
	lunrIndex = lunr(function () {
		this.field("sigla", { boost: 12 });
		this.field("title", { boost: 8 });
		this.field("title_tibetan", { boost: 6 });
		...
		const normalizeTibetan = (token, _tokenIndex, _tokens) => {
...
function populateFilterDropdowns() {
	...
	collections.forEach((collection) => {
		if (collection.genre) {
			categoryCounts[collection.genre] =
				(categoryCounts[collection.genre] || 0) + 1;
		}

		if (collection.place_of_production) {
			placeCounts[collection.place_of_production] =
				(placeCounts[collection.place_of_production] || 0) + 1;
		}
...
function updateDynamicFilterOptions() {
	const currentCollections =
		typeof filteredCollections !== "undefined" &&
	...
	currentCollections.forEach((coll) => {
		...
		if (coll.place_of_production) {
			counts.place[coll.place_of_production] =
				(counts.place[coll.place_of_production] || 0) + 1;
		}
	...
	document.querySelectorAll(".filter-checkbox").forEach((checkbox) => {
		...
		const countSpan = checkbox.parentElement.querySelector(
...
function parseDateString(dateStr) {
	...
	const centuryMatch = dateStr.match(
		/(\d+)(?:st|nd|rd|th|\.)?\s*-\s*(\d+)(?:st|nd|rd|th|\.)?\s*(?:cent|Jh|Jahrhundert|century)/i,
	);
	if (centuryMatch) {
		const startCentury = parseInt(centuryMatch[1], 10);
		const endCentury = parseInt(centuryMatch[2], 10);
		const result = {
			minYear: (startCentury - 1) * 100 + 1,
			maxYear: endCentury * 100,
		};
		return result;
	}

	const singleCenturyMatch = dateStr.match(
		/(\d+)(?:st|nd|rd|th|\.)?\s*(?:cent|Jh|Jahrhundert|century)/i,
	);
	if (singleCenturyMatch) {
		const century = parseInt(singleCenturyMatch[1], 10);
		const result = {
			minYear: (century - 1) * 100 + 1,
			maxYear: century * 100,
		};
		return result;
	}
	...
	const yearRangeMatch = dateStr.match(
		/(?:ca\.?|circa|around|about)?\s*(\d{3,4})\s*-\s*(\d{3,4})/i,
	);
	if (yearRangeMatch) {
		const result = {
			minYear: parseInt(yearRangeMatch[1], 10),
			maxYear: parseInt(yearRangeMatch[2], 10),
		};
		return result;
	}

	const singleYearMatch = dateStr.match(
		/(?:ca\.?|circa|around|about)?\s*(\d{3,4})/i,
	);
	if (singleYearMatch) {
...
function performSearch() {
	performGenericSearch({
		...
		onResultsUpdated: (_results) => {
			document.getElementById("filtered-collections").textContent =
				filteredCollections.length;

			const mappedCollections = filteredCollections.filter(
				(c) => c.coordinates,
			).length;
			document.getElementById("mapped-collections").textContent =
				mappedCollections;
...
function applyFilters() {
	...
	const dateSliderMin = document.getElementById("date-slider-min");
	const dateSliderMax = document.getElementById("date-slider-max");
	const dateMinFilter = dateSliderMin
		? parseInt(dateSliderMin.value, 10)
		: minYear;
	const dateMaxFilter = dateSliderMax
		? parseInt(dateSliderMax.value, 10)
		: maxYear;
	const includeUndatedCheckbox = document.getElementById("include-undated");
	const includeUndated = includeUndatedCheckbox
		? includeUndatedCheckbox.checked
		: true;
...
function updateCollectionsDisplay() {
	...
	if (filteredCollections.length === 0) {
		container.innerHTML =
			'<div class="col-12"><p>No collections match your filters.</p></div>';
		return;
	}
...
function updateHeatmap() {
	...
	if (filteredWithCoords.length === 0) {
		showError(
			"No collections with coordinates to display in heatmap. Try adjusting your filters.",
		);
		return;
	}

	const heatData = filteredWithCoords
		.map((collection) => {
			try {
				...
				let intensity = 0.5;
				if (collection.constituents?.length) {
					intensity += Math.min(collection.constituents.length, 10) * 0.1;
				}
				if (collection.classifications?.length) {
					intensity += collection.classifications.length * 0.15;
				}
				if (collection.date_created) {
					const year = parseInt(collection.date_created, 10);
				...
				console.error(
					`Error creating heat point for ${collection.title}:`,
					error,
				);
				return null;
			}
...
function updateMapWithFiltered() {
	...
	const filteredWithCoords = filteredCollections.filter((c) => c.coordinates);
	const features = filteredWithCoords
		.map((collection) => {
			try {
				const coords = collection.coordinates.split(",");
				const lat = parseFloat(coords[0]);
	...
	const geoJsonLayer = L.geoJSON(
		...
		},
		{
			pointToLayer: (feature, latlng) => {
				...
				if (genre) {
					if (genre.toLowerCase().includes("kanjur"))
						markerColor = "#e41a1c"; // Red
					else if (genre.toLowerCase().includes("tanjur"))
						markerColor = "#377eb8"; // Blue
					else if (genre.toLowerCase().includes("tantra"))
						markerColor = "#4daf4a"; // Green
					else if (genre.toLowerCase().includes("collection"))
						markerColor = "#984ea3"; // Purple
				}
...
function showCollectionDetails(collection) {
	...
	if (collection.notes && Object.keys(collection.notes).length > 0) {
		...
		const filteredNotes = Object.entries(collection.notes).filter(
			([key]) => key !== "integratedrkts" && key !== "isreferenceset",
		);
