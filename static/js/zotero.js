...
const ZOTERO_GROUP_URL =
	"https://www.zotero.org/groups/2296997/rkts_bibliography/library";
...
function initializeZotero() {
	...
	if (viewCards)
		viewCards.addEventListener("click", () => setActiveView("cards"));
	if (viewList) viewList.addEventListener("click", () => setActiveView("list"));
	if (viewCompact)
		viewCompact.addEventListener("click", () => setActiveView("compact"));
...
function setActiveView(view) {
	...
	document
		.querySelectorAll("#view-cards, #view-list, #view-compact")
		.forEach((btn) => {
...
function fetchPublications() {
	...
	if (container) {
		container.innerHTML =
			'<div class="text-center py-4"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><p class="mt-2">Loading publications from Zotero...</p></div>';
	}
	...
	fetch(initialUrl, {
		...
		.then((pages) => {
			...
			if (allItems.length === 0) {
				showError(
					"No publications found. The group might be empty or not publicly accessible.",
				);
				return;
			}
		...
		.catch((error) => {
			showError(
				`Failed to load publications from Zotero: ${error.message || error}`,
			);
			isLoading = false;
		});
}
...
function processPublications(items) {
	...
	if (allPublications.length === 0) {
		showError(
			"No valid publications found after filtering out notes and attachments.",
		);
		return;
	}
...
function showError(message) {
	...
	const container =
		document.getElementById("recent-publications") || document.body;
...
function applyFilters() {
	...
	filteredPublications = allPublications.filter((pub) => {
		...
		if (currentFilters.tag) {
			if (
				!pub.data.tags ||
				!pub.data.tags.some((t) => t.tag === currentFilters.tag)
			) {
				return false;
			}
		}
